...
get_cogs <- function(x, pop) {
  ...
  diff_cog <- lapply(levels(x$source)[-idx], function(a) {
    ...
    tibble(
      ...
      !!paste0("case_abs_death_", id) := cog(abs(rcd - cd),
        desc = paste0("Absolute difference between ",
  ...
  extra_cog <- tibble(
    case_increase_pct = cog(ifelse(rpc == 0, NA, 100 * rnc / rpc),
      desc = paste0("% increase in cases (", ref_source, ")")),
    death_increase_pct = cog(ifelse(rpd == 0, NA, 100 * rnd / rpd),
      desc = paste0("% increase in deaths (", ref_source, ")")),
    new_case_change_pct = cog(ifelse(rnpc == 0, NA, 100 * (rnc - rnpc) / rnpc),
      desc = paste0("% day-to-day change in new cases (", ref_source, ")")),
    new_death_change_pct = cog(ifelse(rnpd == 0, NA, 100 * (rnd - rnpd) / rnpd),
    ...
    case_fatality_pct = cog(chk(round(100 * rcd / rcc, 1)),
